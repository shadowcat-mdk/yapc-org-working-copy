...
* \L{url="../Getting-started-slides/frame_set.html" target=_blank}<Getting Started>

* \L{url="../FAQ-pp2html-slides/frame_set.html" target=_blank}<pp2html FAQ>

* \L{url="../FAQ-parser-slides/frame_set.html" target=_blank}<PerlPoint Parser FAQ>

* \L{url="../wrco/frame_set.html" target=_blank}<Writing PerlPoint converters>
...
All possible options are described in the \SECTIONREF{name="Options"} chapter.
...
\INCLUDE{type=PP file="tagdoc-supported.pp" headlinebase=2}
...
documents. Just write down all options you need in file, line by line:
...
This means that all tokens are printed on \B<STDOUT> so that you can easily see, where the
parser stops.
...
* Perl Projector from Clinton Pierce <clintp@geeksalad.org>
